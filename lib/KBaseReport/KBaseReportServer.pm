package KBaseReport::KBaseReportServer;
...
sub call_method {
    ...
    my $ctx = KBaseReport::KBaseReportServerContext->new($self->{loggers}->{userlog},
...
{
    # Service KBaseReport requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for KBaseReport but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = KBaseReport::KBaseReportServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service KBaseReport.");
    }
...
package KBaseReport::KBaseReportServerContext;

use strict;

=head1 NAME

KBaseReport::KBaseReportServerContext
...
package KBaseReport::KBaseReportServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = KBaseReport::KBaseReportServer->new(
