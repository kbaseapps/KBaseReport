...
class KBaseReport:
    ...
    GIT_COMMIT_HASH = "d9fbb1ec68ea13ac3cba79d4dfed999fa03b7751"
    ...
    def __init__(self, config):
        ...
        self.template_toolkit_config = {}
        config_parser = ConfigParser()
        config_file = os.environ.get('KB_DEPLOYMENT_CONFIG', None)
        if (config_file):
            config_parser.read(config_file)
            for config_item in config_parser.items('TemplateToolkitPython'):
                self.template_toolkit_config[ config_item[0] ] = config_item[1]

        self.config['template_toolkit'] = self.template_toolkit_config
    ...
    def create_extended_report(self, ctx, params):
        ...
        :param params: instance of type "CreateExtendedReportParams" (*
           ...
           set the direct/default view in the *         report. Set only one
           of 'direct_html', 'template', and 'direct_html_link_index'. *    
           string direct_html - Simple HTML text content that will be
           rendered within the report *         widget. Set only one of
           'direct_html', 'template', and 'direct_html_link_index'. *        
           Setting both 'template' and 'direct_html' will generate an error.
           *     TemplateParams template - render a template to produce HTML
           text content that will be *         rendered within the report
           widget. Setting both 'template' and 'direct_html' will *        
           generate an error. *     list<File> file_links - A list of file
           paths or shock node IDs. Allows the user to *         specify
           files that the report widget should link for download. If you pass
           in paths *         to directories, they will be zipped *    
           ...
           * Required arguments: *     string path - Can be a file or
           ...
           "template" of type "TemplateParams" -> structure: parameter
           ...
           *     string path - Can be a file or directory path. Required if
           shock_id is absent *     string shock_id - Shock node ID. Required
           if path is absent *     string name - Plain-text filename (eg.
           "results.zip") -- shown to the user * Optional arguments: *    
        ...
        if 'template' in params:
            ...
            params = template_utils.render_template_to_direct_html(params, self.config)
    ...
    def render_template(self, ctx, params):
        ...
        output_file_path = template_utils.render_template_to_file(params, self.config)
        #END render_template
